# === main.py (Bitget SPOT — *_SPBL only, products-driven) ===
import os, time, hmac, hashlib, base64, json, threading, logging
from flask import Flask
import requests

# -------- KEYS (from your setup) --------
...
# ---- products: pick EXACT *_SPBL symbols ----
def fetch_spot_spbl_symbols():
    ...
    for p in r.get("data", []):
        ...
        sym=ensure_spbl(p.get("symbol") or "")
        if base in DESIRED_BASES and quote==QUOTE:
            syms.append(sym)
    return sorted(set(syms))

# ---- market data & orders: *_SPBL ONLY ----
...
def get_price(symbol_spbl):
    ...
    if r2.get("code") == "00000" and r2.get("data"):
        d=r2["data"][0]; p=safe_float(d.get("lastPr") or d.get("last"))
        if p is not None: return p
...
def trade_loop():
    ...
    SYMBOLS = fetch_spot_spbl_symbols()
    if not SYMBOLS:
        tg("❗ products не вернул пары *_SPBL для USDT."); return
    last_no_signal = {s:0 for s in SYMBOLS}
    tg("🤖 Bitget SPOT (SPBL-only). Пары:\n" + ", ".join(SYMBOLS))

    # self-test
    for s in SYMBOLS:
        try:
            p = get_price(s); _ = get_candles(s, 30)
...
def home():
    return "Bitget SPOT bot (SPBL-only) is running", 200
