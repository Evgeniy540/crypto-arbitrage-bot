...
# === ТВОИ ДАННЫЕ (подставлены из твоих сообщений) ===
TELEGRAM_BOT_TOKEN = "7630671081:AAG17gVyITruoH_CYreudyTBm5RTpvNgwMA"
TELEGRAM_CHAT_ID   = "5723086631"
# =====================================================

# Расширенный список символов KuCoin (формат с дефисом!)
DEFAULT_SYMBOLS = [
...
# Строгие дефолт-тайминги (чтобы не «штормило» и не ловить лимиты)
CHECK_INTERVAL_S     = 20     # частота цикла (используется как пауза между батчами)
SIGNAL_COOLDOWN_S    = 300    # минимум 5 минут между сигналами по одной монете
...
HEADERS        = {"User-Agent": "ema-kucoin-bot/2.3"}
...
state = {
    ...
    # стартуем в тихом профиле (см. apply_preset_quiet)
    "eps_pct": 0.0010,
    "atr_k": 0.25,
    "slope_min": 0.0,
    ...
    # анти-лимиты KuCoin
    "batch_size": 6,        # сколько монет проверяем за один цикл
    "per_req_sleep": 0.25,  # пауза между REST-запросами (сек)
    "rr_index": 0,          # round-robin указатель
    "max_retries": 3,       # повторы при 429/сетевых
    "backoff_base": 0.7,    # базовая пауза экспоненциального бэкоффа
}
...
# ===== Анти-лимитная обёртка =====
def kucoin_get(url, params, timeout=10):
    """Ретраи + экспоненциальный бэкофф для 429/сетевых ошибок."""
    tries = 0
    while True:
        tries += 1
        try:
...
def fetch_candles(symbol, tf, want=300):
    ...
    # лёгкий троттлинг между вызовами REST
...
def decide_signal(e9, e21, atr_arr, price, eps_pct, atr_k, slope_min):
    ...
    if v:
        ...
        # строгий ATR-фильтр для quiet/normal (atr_k >= 0.20)
    ...
    if v:
        if atr_arr and atr_arr[-1] and atr_k >= 0.20:  # ужимаем отскоки в тихом профиле
...
def next_symbols_batch():
    """Возвращает срез монет для текущего цикла, сдвигая rr_index (round-robin)."""
    syms = state["symbols"]
    if not syms: return []
...
def handle_cmd(text):
    if text.startswith("/mode"):
        ...
        apply_preset_quiet(); send_tg("🤫 QUIET preset (минимум сигналов)")
        ...
        try:
            ...
            # сброс rr_index, чтобы батч начинался с начала
            state["rr_index"]=0
...
def signals_worker():
    send_tg("🤖 KuCoin EMA бот (anti-rate-limit ON, quiet) запущен. /help")
    while True:
        try:
...
if __name__=="__main__":
    # применим тихий пресет на старте
    apply_preset_quiet()
